...
class FilterTrackConsumer extends OauthPhirehose {
    ...
    private $tweetDelimiter = 0; //made by wei
    ...
    public function enqueueStatus($status) {
        ...
        if (is_array($data) && isset($data['user']['screen_name'])) {
            ...
            if ($this->tweetDelimiter === 0) {
                print "\n" . date("Y-m-d H:i:s") . ':  ' . $data['user']['screen_name'] . ': ' . $textData;
                $this->tweetDelimiter++;
            } else {
                print $data['user']['screen_name'] . ': ' . $textData;
            }
            $this->found_flag = 1;
        }
    }
    ...
    public function db_connect() {
        if ($this->mysqli == NULL) {
            ...
            if ($this->mysqli->connect_error) {
                ...
                print "\n" . date("Y-m-d H:i:s") . "Database Connected!!\n";
